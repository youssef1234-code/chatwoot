class Messages::MentionService
  ...
  def filter_mentioned_ids_by_inbox
    inbox = message.inbox
    # Channel-restricted admins must not be mentionable in inboxes outside their allow-list.
    valid_mentionable_ids = inbox.administrators_with_access.map(&:id) + inbox.members.map(&:id)
    # Intersection of ids
