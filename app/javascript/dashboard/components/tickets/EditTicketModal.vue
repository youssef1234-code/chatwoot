        ...
        <div class="flex-1 overflow-y-auto p-6">
          <form @submit.prevent="updateTicket" class="space-y-4">
            ...
            <div>
              ...
              />
              <p v-if="errors.issue_type" class="mt-1 text-sm text-red-600">{{ errors.issue_type[0] }}</p>
            </div>
...
const loadJiraIssues = async () => {
  ...
  try {
    const response = await JiraAPI.getLinkedIssues(props.ticket.conversation_id);
