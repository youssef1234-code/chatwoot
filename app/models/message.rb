...
#  index_messages_on_conversation_pinned                (conversation_id,is_pinned)
#  index_messages_on_conversation_starred               (conversation_id,is_starred)
#  index_messages_on_created_at                         (created_at)
#  index_messages_on_inbox_id                           (inbox_id)
#  index_messages_on_pinned_by                          (pinned_by)
#  index_messages_on_sender_type_and_sender_id          (sender_type,sender_id)
#  index_messages_on_source_id                          (source_id)
#  index_messages_on_starred_by                         (starred_by)
#
# Foreign Keys
#
#  fk_rails_...  (pinned_by => users.id)
#  fk_rails_...  (starred_by => users.id)
#
...
class Message < ApplicationRecord
  ...
  scope :starred_in_conversation, ->(conversation) { where(conversation: conversation, is_starred: true) }
  scope :pinned_in_conversation, ->(conversation) { where(conversation: conversation, is_pinned: true) }
  ...
  belongs_to :starred_by_user, class_name: 'User', foreign_key: 'starred_by', optional: true
  belongs_to :pinned_by_user, class_name: 'User', foreign_key: 'pinned_by', optional: true

  has_many :attachments, dependent: :destroy, autosave: true, before_add: :validate_attachments_limit
  has_one :csat_survey_response, dependent: :destroy_async
  has_one :nps_survey_response, dependent: :destroy_async
  has_many :notifications, as: :primary_actor, dependent: :destroy_async
  has_many :ticket_messages, dependent: :destroy_async
  has_many :tickets, through: :ticket_messages
  ...
  def star!(user)
    update!(is_starred: true, starred_by: user.id, starred_at: Time.current)
  end

  def unstar!
    update!(is_starred: false, starred_by: nil, starred_at: nil)
  end

  def pin!(user)
    # Unpin any other pinned messages in the same conversation to ensure only one is pinned
    Message.where(conversation: conversation, is_pinned: true).update_all(
      is_pinned: false, 
      pinned_by: nil, 
      pinned_at: nil
    )
    update!(is_pinned: true, pinned_by: user.id, pinned_at: Time.current)
  end

  def unpin!
    update!(is_pinned: false, pinned_by: nil, pinned_at: nil)
  end
