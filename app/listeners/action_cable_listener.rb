class ActionCableListener < BaseListener
  ...
  def message_created(event)
    ...
    tokens = user_tokens(account, conversation.inbox.members, conversation.inbox_id) + contact_tokens(conversation.contact_inbox, message)
  ...
  def first_reply_created(event)
    message, account = extract_message_and_account(event)
    conversation = message.conversation
    tokens = user_tokens(account, conversation.inbox.members, conversation.inbox_id)
  ...
  def conversation_created(event)
    ...
    tokens = user_tokens(account, conversation.inbox.members, conversation.inbox_id) + contact_inbox_tokens(conversation.contact_inbox)
  ...
  def typing_event_listener_tokens(account, conversation, user)
    ...
    (user_tokens(account, conversation.inbox.members, conversation.inbox_id) + [conversation.contact_inbox.pubsub_token]) - [current_user_token]
  end

  def user_tokens(account, agents, inbox_id = nil)
    agent_tokens = agents.pluck(:pubsub_token)
    admin_tokens = admin_tokens_for_inbox(account, inbox_id)
    (agent_tokens + admin_tokens).uniq
  end

  # Admins receive realtime updates for the conversation's inbox — but a
  # channel-restricted admin must NOT receive events for inboxes outside their
  # allow-list. When inbox_id is nil (account-level events) all admins are included.
  def admin_tokens_for_inbox(account, inbox_id)
    account_users = account.account_users.where(role: AccountUser.roles[:administrator]).includes(:user)
    account_users.filter_map do |account_user|
      next if inbox_id.present? && !account_user.can_access_inbox?(inbox_id)

      account_user.user&.pubsub_token
    end
  end
